...
export default function MailDetail() {
	...
	if (!mail) return <p className="text-gray-500 font-semibold">{t("mail.loading")}</p>

	return (
		<div className="max-w-4xl mx-auto">
			{/* Header */}
			<div className="sticky top-14 bg-gray-100 dark:bg-gray-900 py-2 flex items-center gap-2">
				<button onClick={() => navigate(-1)} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800" title={t("common.back")}>
					<ArrowLeft size={18}/>
				</button>
			...
			<div className="bg-white dark:bg-gray-800 mt-4 p-4 rounded shadow">
				<div className="mb-3">
					<p className="text-sm text-gray-500">
						{t("mail.to")}:{" "} <span className="font-medium">{mail.receiver_mail}</span>
					</p>
				...
				{mail.attachments?.length > 0 && (
					<div className="mt-4">
						<h3 className="text-sm font-semibold flex items-center gap-2">
							<Paperclip size={16}/> {t("mail.attachments")}
						</h3>
